...
public sealed class MainMenuStarDisplay : MonoBehaviour
{
    ...
    public static TMP_Text ResolveStarCountText(Transform searchRoot)
    {
        ...
        if (searchRoot != null)
        {
            text = FindStarTextNear(searchRoot)
                   ?? FindStarTextNear(searchRoot.root)
                   ?? FindTextAtAnyPath(searchRoot, StarTextPaths)
                   ?? FindTextAtAnyPath(searchRoot.root, StarTextPaths);
        }
